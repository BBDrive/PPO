...
class EnvWorker(mp.Process):
    ...
    def try_reset(self):
        with self.lock:
            self.task_id = self.queue.get(True)  # True代表当队列空了之后，get 就会阻塞，一直等待队列中有数据后再获取数据, False则直接报错（当队列空时）
    ...
    def run(self):
        while True:
            ...
            if command == 'step':
                ...
                self.remote.send((observation.reshape(-1), reward, done, self.task_id, info))
            elif command == 'reset':
                observation = self.try_reset()
                self.remote.send((observation.reshape(-1), self.task_id))
